...
class get_model(nn.Module):
    def __init__(self, angRes=5, factor=4):
        super(get_model, self).__init__()
        n_blocks, channel = 4, 32
        self.factor = factor
        self.angRes = 5
        self.FeaExtract = FeaExtract(channel)
        self.ADAM_1 = ADAM(channel, 5)
        self.ADAM_2 = ADAM(channel, 5)
        self.ADAM_3 = ADAM(channel, 5)
...
class ADAM(nn.Module):
    def __init__(self, channel, angRes):
        ...
        self.init_offset()
        self.conv_f1 = nn.Conv2d(5*5*channel, 5*5*channel, kernel_size=1, stride=1, padding=0)

        self.conv_f3 = nn.Conv2d(2*channel, channel, kernel_size=1, stride=1, padding=0)
        self.dcn = DCN(channel, channel, 3, 1, 1, deformable_groups=1)
