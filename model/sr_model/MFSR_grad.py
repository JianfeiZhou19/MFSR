...
class DisentgBlock(nn.Module):
    def __init__(self, angRes, channels):
        ...
        SpaChannel, AngChannel, EpiChannel, all_channels = channels, channels//4, channels//2, channels
        ...
        self.all = nn.Sequential(
            nn.Conv3d(channels, all_channels, kernel_size=3, padding=1, stride=1, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv3d(all_channels, all_channels, kernel_size=3, padding=1, stride=1, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
        )
        self.fuse = nn.Sequential(
            nn.Conv2d(SpaChannel + AngChannel + 2 * EpiChannel+all_channels, channels, kernel_size=1, stride=1, padding=0, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(channels, channels, kernel_size=3, stride=1, dilation=5, padding=5, bias=False),
        )

    def forward(self, x):
        # all_in = MacPI2SAI(x, angRes=5)
        all_in = rearrange(x, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=5, a2=5)
        all_out = self.all(all_in)
        all_out = rearrange(all_out, 'b c (a1 a2) h w -> b c (a1 h) (a2 w)', a1=5, a2=5)
        # all_out = SAI2MacPI(all_out, angRes=5)
        feaSpa = self.SpaConv(x)
        feaAng = self.AngConv(x)
        feaEpiH = self.EPIConv_h(x)
        feaEpiV = self.EPIConv_v(x.permute(0, 1, 3, 2).contiguous()).permute(0, 1, 3, 2)
        buffer = torch.cat((feaSpa, feaAng, feaEpiH, feaEpiV, all_out), dim=1)
...
class get_loss(nn.Module):
    ...
    def forward(self, SR, HR, criterion_data=[]):
        l1_loss = self.criterion_Loss(SR, HR)
        gradient_loss = self.gradient_loss(HR, SR)
        loss = {"l1_loss":l1_loss, "gradient_loss":gradient_loss}
        return loss
